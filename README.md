...
[![Tests](https://img.shields.io/badge/tests-808%20pass-brightgreen)](src/)
...
| パッケージ   | pnpm 10.26.2                                                                      |
| テスト       | 808 core tests＋24 Product Assurance tests＋Playwright E2E 28 tests               |
...
| `GET`    | `/api/v1/governance/audit/verify` | `audit:read`        | 監査ハッシュチェーン整合性の検証。`{ valid, brokenAt?, checkedAt }` + `X-CEOP-Audit-Valid` ヘッダ。検証結果自体も監査記録される                                                                              |
...
# Product Assuranceを含む全自動Release gate
pnpm run verify

# Golden / Logic / Data / Resilience / AI Eval
pnpm run quality:merge

# 固定データから構造化Evidenceを生成
CEOP_TEST_EVIDENCE_FILE=artifacts/evidence.jsonl pnpm run quality:merge
pnpm run quality:evidence -- artifacts/evidence.jsonl artifacts/evidence-summary.json
...
| [Product Assurance戦略](docs/quality/TEST_STRATEGY.md)      | SoftwareからHuman Acceptanceまでの品質モデル   |
| [Test Matrix](docs/quality/TEST_MATRIX.md)                  | Business RiskとTest/Evidence/実行Stageの対応   |
| [Release Criteria](docs/quality/RELEASE_CRITERIA.md)        | 信用できるProductのRelease条件                 |
...
「CEOP Platform」）。`docker compose --profile monitoring up -d` で Prometheus(19090) +
Grafana(13001) を loopback（host network）起動できます（実機導入は RUNBOOK 参照）。
