...
| 週次   | ログ量・ディスク使用率確認             | 運用         | 80% 超で容量増強計画。コンテナログはローテーション未設定のため `docker inspect ceop-platform --format '{{.LogPath}}'` のサイズも確認                                   |
...
日次項目が人手なのは、**通知経路が未実装**だからです（`MONITORING.md` §2）。
`scripts/health-probe.sh` は連続失敗を検知して終了コード 1 と `ALERT` 行を出しますが、
それを受け取って人へ届ける先（PagerDuty / Slack / メール）が決まっていません。
