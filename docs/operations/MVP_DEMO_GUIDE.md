...
pnpm run test:e2e        # Playwright（portal/dashboard/daily-reports/iso-app/mvp-demo・chromium で 28 テスト）
```

CI は `playwright.config.ts` の `projects` で Chromium（全スペック）に加え、Firefox・WebKit・
モバイルビューポート（Pixel 7 / iPhone 14）でログイン・ダッシュボード表示のスモークテストを
実行します（`.github/workflows/ci.yml` の `e2e` ジョブ、ブラウザごとにマトリクス実行）。

※ 開発機で Chromium が SIGTRAP になる環境では、ローカル限定設定
`playwright.firefox.local.config.ts`（`--config` で指定）で Firefox 上に全スペックを実行できます
（この設定は CI では使用しません）。
