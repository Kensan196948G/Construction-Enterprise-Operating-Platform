...
| E-04 | 承認・ワークフロー | 変更申請・電子署名・決裁滞留 | 📦 workflow-instance（P2 で移植） | ⬜ |
...
| Y-02 | Issue 管理 | Issue オブジェクト CRUD | 📦 issue ドメイン（workflow-instance と統合） | ⬜ |
| Y-03 | Document | 文書オブジェクト CRUD | 📦 document ドメイン | ⬜ |
| Y-04 | Approval | 申請→承認→監査フロー | 📦 workflow-instance API（P2 で移植） | ⬜ |
