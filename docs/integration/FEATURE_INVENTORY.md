...
| #    | 機能         | 内容                                                   | CEOP 対応                                  | 状態 |
| ---- | ------------ | ------------------------------------------------------ | ------------------------------------------ | ---- |
| L-01 | 写真ログ登録 | 案件番号/材料名/数量/置場/座標のメタデータ CRUD        | 📦 material-photo-log ドメイン + API + 監査 | ✅   |
| L-02 | CSV エクスポート | 固定カラム順の CSV 出力（Excel 台帳取込用）          | 📦 /api/v1/material-photo-logs/export.csv | ✅   |
