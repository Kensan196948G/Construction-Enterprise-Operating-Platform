...
- テスト 11 件追加 → **382/382 pass**・verify/build/audit 0
...
- テスト 10 件追加 → **382/382 pass**・verify/build/audit 0
...
- テスト 5 件追加 → **382/382 pass**・verify/build/audit 0
...
- テスト 4 件追加 → **382/382 pass**・verify/build/audit 0
...
- テスト 2 件追加 → **382/382 pass**・verify/build/audit 0
- PR #33（feat/p3-e-slices）

### P3 完了スライス: S-07 残・E-11 残（2026-08-07 本セッション追記）

- `compliance.ts`（S-07: ComplianceCheck + LegalEvidence）・`notification-template.ts`（E-11）
- API: compliance-checks / legal-evidence / notification-templates / notifications unread-count・read
...
- テスト 6 件追加 → **382/382 pass**・verify/build/audit 0
...
### P5: パリティ検証・本番切替（v0.10.0・2026-08-07 本セッション追記）

- `scripts/verify-parity.ts`: FEATURE_INVENTORY 突合 + API プローブ 24 ルート + migration 001〜024
  カバレッジ検査（pnpm run verify に組込）
- FEATURE_INVENTORY: Y-03/Y-07・O-02/O-03/O-05・D-08 を ✅ に更新し、次期バックログ
  （O-04/O-06/O-08/O-09/D-04/D-07/D-09）を明示
- **発見・修正**: P3/P4 の migration 016〜024 がマージ過程で欠落 → 復元（PR #37）+
  parity 検査で再発防止
- v0.10.0 リリース: tag（a005ab6・修正版）・Release workflow success（GHCR 0.10.0）
- 本番切替:
  - 事前バックアップ ceop-predeploy-v0.10.0-*.db
  - migration 001〜024 適用（016〜024 を追加適用・9 業務テーブル確認）
  - コンテナ差し替え ceop-platform:v0.10.0（旧 v0.9.0 は ceop-platform-prev-v090 保持）
  - スモーク: /health 200・info=0.10.0・/metrics 200・/portal 200・protected 401・WebUI/公開 200
- 監視スタック導入: Prometheus(19090)/Grafana(13001) host network・loopback bind・read-only・
