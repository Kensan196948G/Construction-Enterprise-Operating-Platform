...
async function main() {
  ...
  if (!hasTestEvidence) {
    ...
    const hasTests =
      ...
      (() => {
        try {
          return !!JSON.parse(fs.readFileSync(packageJson, 'utf8')).scripts?.test;
        } catch (_) {
          return false;
        }
      })();
...
main().catch((e) => {
  console.error(`[TaskCompleted Gate] 内部エラー: ${e?.message || e}`);
  process.stdout.write(
    '[TaskCompleted Gate] 検証に失敗したため、完了を一時ブロックしました。再実行してください。\n',
  );
  process.exit(2);
});
