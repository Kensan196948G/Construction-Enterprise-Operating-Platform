...
async function main() {
  ...
  const taskDirs = [path.join(process.env.HOME || '/root', '.claude', 'tasks')];
  let openTaskCount = 0;
  ...
  // Keep working if open tasks remain
  if (openTaskCount > 0) {
    process.stdout.write(
      `[TeammateIdle Gate] ${teammateName}: 未完了タスクが ${openTaskCount} 件残っています。` +
        `タスクを完了してから終了してください。\n`,
    );
    process.exit(2);
  }
...
main().catch((e) => {
  console.error(`[TeammateIdle Gate] 内部エラー: ${e?.message || e}`);
  process.stdout.write(
    '[TeammateIdle Gate] 検証に失敗したため、アイドルを一時ブロックしました。再実行してください。\n',
  );
  process.exit(2);
});
