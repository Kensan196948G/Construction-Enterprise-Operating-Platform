...
グローバル設定（`~/.Codex/AGENTS.md`）の方針を継承しつつ、プロジェクト固有の設定を定義します。

本システムは以下として統合動作する：

- AI開発組織そのもの（CTO・開発・QA・Security・CI/CD・PM を一体化）
- `/goal` コマンド駆動の自律継続開発（Codex v2.1.139+ 公式機能）
- Agent Teams による並列協調開発（Experimental）
- Agent View（`Codex agents`）によるセッション監視
- GitHub 連携による完全無人運用
...
- **Codex の有無は停止条件ではない。** Codex 単独でも自律開発を継続すること
...
- 条件は Codex が会話内で出力できる形で書く（テスト実行結果・CI 状態など）
- 1 セッション 1 ゴールを原則とする
- `or stop after N turns` をターン上限として含める（無限継続防止）
- `/goal clear` で即時クリア、`/goal` 単体で進捗確認
- 非対話モード（cron）: `Codex -p "/goal <条件>"` で動作
...
- 👁 **`Codex agents` で Agent View を起動し、セッション状態を監視すること**
- 📊 全プロセスや状況を可視化すること
...
| 🔧 設定・ファイル・構成 | 🔧 ⚙️ 📁 📄 🛠️ | `🔧 settings.json 更新` `📁 .Codex/` |
...
| /goal 公式 docs | `https://code.Codex.com/docs/en/goal` |
