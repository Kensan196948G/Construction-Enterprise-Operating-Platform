...
function Sidebar({ currentPath, onNavigate, collapsed, onToggle, activeRole }) {
  const [openCategories, setOpenCategories] = React.useState(() => {
    const defaults = activeRole && ROLE_DEFAULTS[activeRole] ? ROLE_DEFAULTS[activeRole] : [];
    const map = {};
  ...
  const sectionMatchesQuery = (section) => {
    if (!normalizedQuery) return true;
    return includesQuery(section.label) || section.pages.some(page => includesQuery(page.label) || includesQuery(page.href));
  };
  const categoryMatchesQuery = (cat) => {
    if (!normalizedQuery) return true;
    return includesQuery(cat.label) || cat.sections.some(sectionMatchesQuery);
  };

  const filteredMenu = MENU_STRUCTURE
    .filter(categoryMatchesQuery)
    .map(cat => ({
      ...cat,
      sections: cat.sections
        .filter(sec => isVisible(sec) && sectionMatchesQuery(sec))
        .map(sec => ({
          ...sec,
          pages: normalizedQuery && !includesQuery(sec.label) && !includesQuery(cat.label)
            ? sec.pages.filter(page => includesQuery(page.label) || includesQuery(page.href))
            : sec.pages,
        }))
        .filter(sec => sec.pages.length > 0 || includesQuery(sec.label) || includesQuery(cat.label)),
    }))
    .filter(cat => cat.sections.length > 0);

  const sidebarStyle = {
    display: 'flex', flexDirection: 'column', height: '100%',
    background: 'var(--sidebar-bg, #0f172a)', color: '#94a3b8',
    ...
    <aside style={sidebarStyle}>
      ...
      {!collapsed && (
        <div style={{
          ...
          <div style={{
            ...
            <Icon name="search" size={14} style={{ color: '#64748b' }} />
            <input
              value={menuQuery}
      ...
      <nav style={{
        ...
        {filteredMenu.map(cat => {
          const isOpen = normalizedQuery ? true : openCategories[cat.id];
          const hasActive = cat.sections.some(s => s.pages.some(p => p.href === currentPath));
          ...
          return (
            <div key={cat.id} style={{ marginBottom: 2 }}>
              <button onClick={() => toggleCat(cat.id)} style={{
                ...
                border: 'none', background: hasActive ? '#1e293b' : 'transparent',
                color: hasActive ? '#e2e8f0' : '#94a3b8',
                cursor: 'pointer', fontSize: 13, fontWeight: 600,
                transition: 'all 0.15s', textAlign: 'left',
                fontFamily: 'inherit',
              }}
                onMouseEnter={e => { if (!hasActive) e.currentTarget.style.background = '#1e293b'; }}
                onMouseLeave={e => { if (!hasActive) e.currentTarget.style.background = 'transparent'; }}
              >
                <Icon name={cat.icon} size={18} style={{ color: hasActive ? '#60a5fa' : '#64748b' }} />
                ...
                {cat.sections.some(s => s.pages.some(p => p.badge)) && (
                  <span style={{
                    ...
                    {cat.sections.flatMap(s => s.pages).filter(p => p.badge).reduce((a, p) => {
                      const n = parseInt(p.badge); return a + (isNaN(n) ? 1 : n);
                    }, 0)}
                  </span>
                )}
              ...
              <div style={{
                ...
                {cat.sections.filter(isVisible).map(sec => {
                  const secOpen = normalizedQuery ? true : openSections[sec.id];
                  const secHasActive = sec.pages.some(p => p.href === currentPath);
                  ...
                  return (
                    <div key={sec.id} style={{ marginLeft: 12, marginTop: 2 }}>
                      <button onClick={() => toggleSec(sec.id)} style={{
                        ...
                        onMouseLeave={e => e.currentTarget.style.color = secHasActive ? '#bfdbfe' : '#64748b'}
                      >
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                          strokeWidth="2.5" style={{
                            transition: 'transform 0.2s', transform: secOpen ? 'rotate(90deg)' : 'rotate(0)',
                          }}>
                          <polyline points="9 18 15 12 9 6" />
                        </svg>
                        <span>{sec.label}</span>
                      ...
                      <div style={{
                        ...
                        {sec.pages.map(page => {
                          ...
                          return (
                            <button key={page.id} onClick={() => onNavigate(page.href)}
                              style={{
                                display: 'flex', alignItems: 'center', gap: 8,
                            ...
                            >
                              ...
                              {page.badge && (
                                <span style={{
                                  background: page.badge === '!' ? '#f97316' : '#ef4444',
                                  color: '#fff', fontSize: 9, fontWeight: 700,
                                  padding: '0px 5px', borderRadius: 8, minWidth: 16,
      ...
      <button onClick={onToggle} style={{
        position: 'absolute', top: 62, right: -12,
      ...
      <div style={{
        ...
        <div style={{
          ...
          color: '#93c5fd', fontSize: 12, fontWeight: 700, flexShrink: 0,
        }}>TK</div>
        {!collapsed && (
          <div style={{ overflow: 'hidden' }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#e2e8f0', whiteSpace: 'nowrap' }}>田中 健一</div>
            <div style={{ fontSize: 10, color: '#64748b' }}>
              {activeRole && ROLES[activeRole] ? ROLES[activeRole].label : '現場監督'}
            </div>
          </div>
        )}
      </div>
