...
function FieldProjectsView() {
  ...
  const projects = [
    {
      name: '品川タワー新築工事', client: '品川都市開発(株)',
      period: '2025/04 〜 2027/03', progress: 68, status: '施工中',
      workers: 45, equipment: 8, weather: '晴れ 32°C',
      phases: [
      ...
      alerts: [
        { type: 'warning', message: '明日15時以降 強風予報（風速12m/s）— クレーン作業中止検討' },
        { type: 'info', message: 'AI工程分析：鉄骨建方は予定通り完了見込み' },
      ],
    },
    {
      name: '横浜分譲マンション', client: '横浜住宅(株)',
      period: '2025/06 〜 2027/09', progress: 42, status: '施工中',
      workers: 32, equipment: 5, weather: '曇り 28°C',
      phases: [
        { name: '基礎工事', progress: 100, status: 'done' },
      ...
      alerts: [
        { type: 'danger', message: '足場不備を検知 — AI画像解析により是正勧告' },
      ],
    },
  ];
  ...
  const statusDot = (s) => ({
    done: '#16a34a', active: '#1a56db', upcoming: '#d1d5db',
  }[s]);
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: '#0f172a', margin: 0 }}>現場DX</h1>
          <p style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>工事管理・現場進捗・安全管理</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button style={{
            ...
            background: '#1a56db', color: '#fff', fontSize: 12, fontWeight: 600,
            cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6,
            fontFamily: 'inherit',
          }}>
            <Icon name="plus" size={14} />新規工事登録
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20 }}>
        ...
        <div style={{ ...cardStyle, padding: 0 }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#0f172a' }}>工事一覧</div>
          </div>
          <div>
            {projects.map((p, i) => (
              <div key={i} onClick={() => setSelectedProject(i)} style={{
                padding: '14px 16px', cursor: 'pointer',
                background: i === selectedProject ? '#eff6ff' : 'transparent',
                borderBottom: '1px solid #f1f5f9',
                borderLeft: i === selectedProject ? '3px solid #1a56db' : '3px solid transparent',
                transition: 'all 0.12s',
              }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#0f172a' }}>{p.name}</div>
                <div style={{ fontSize: 11, color: '#64748b', marginTop: 3 }}>{p.client}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
                  <div style={{ flex: 1, height: 4, background: '#e2e8f0', borderRadius: 2, overflow: 'hidden' }}>
                    <div style={{
                      height: '100%', width: `${p.progress}%`, borderRadius: 2,
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 16 }}>
            {[
              { label: '進捗率', value: `${proj.progress}%`, icon: 'activity', color: '#1a56db', bg: '#eff6ff' },
              { label: '作業員', value: `${proj.workers}名`, icon: 'users', color: '#7c3aed', bg: '#f5f3ff' },
              { label: '重機稼働', value: `${proj.equipment}台`, icon: 'hard-hat', color: '#f97316', bg: '#fff7ed' },
              { label: '天候', value: proj.weather, icon: 'sun', color: '#eab308', bg: '#fefce8' },
            ].map((s, i) => (
          ...
          {proj.alerts.map((a, i) => (
            <div key={i} style={{
              ...
              background: a.type === 'danger' ? '#fef2f2' : a.type === 'warning' ? '#fff7ed' : '#eff6ff',
              border: `1px solid ${a.type === 'danger' ? '#fecaca' : a.type === 'warning' ? '#fed7aa' : '#bfdbfe'}`,
              display: 'flex', alignItems: 'center', gap: 10,
              fontSize: 12, color: a.type === 'danger' ? '#991b1b' : a.type === 'warning' ? '#92400e' : '#1e3a8a',
            }}>
            ...
            <div style={{ padding: 20 }}>
                    ...
                    {proj.todayTasks.map((t, i) => (
                      <div key={i} style={{
                        ...
                        <div style={{
                          ...
                          border: '2px solid #fff', boxShadow: '0 0 0 2px ' + statusDot(t.status) + '40',
                          marginTop: 2,
                        }}></div>
                        <div>
                          <div style={{
                          ...
                          <div style={{
                            fontSize: 13, fontWeight: t.status === 'active' ? 600 : 400,
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {proj.phases.map((ph, i) => (
                      <div key={i} style={{
                        ...
                        background: ph.status === 'active' ? '#eff6ff' : '#f8fafc',
                        border: `1px solid ${ph.status === 'active' ? '#bfdbfe' : '#e2e8f0'}`,
                      }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                          <span style={{ fontSize: 13, fontWeight: 600, color: '#0f172a' }}>{ph.name}</span>
                          <span style={{
                            fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 10,
                            background: ph.status === 'done' ? '#dcfce7' : ph.status === 'active' ? '#dbeafe' : '#f1f5f9',
                            color: ph.status === 'done' ? '#166534' : ph.status === 'active' ? '#1e40af' : '#94a3b8',
                          }}>
                            {ph.status === 'done' ? '完了' : ph.status === 'active' ? '進行中' : '未着手'}
                          </span>
                        </div>
                        <div style={{ height: 6, background: '#e2e8f0', borderRadius: 3, overflow: 'hidden' }}>
                          <div style={{
                            height: '100%', width: `${ph.progress}%`, borderRadius: 3,
                            background: ph.status === 'done' ? '#16a34a' : '#1a56db',
                            transition: 'width 0.8s',
                          }}></div>
                        </div>
                        <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 4 }}>{ph.progress}% 完了</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {activeTab === 'photos' && (
                <div style={{ textAlign: 'center', padding: 40, color: '#94a3b8' }}>
                  <Icon name="camera" size={40} style={{ color: '#d1d5db', margin: '0 auto 12px' }} />
                  <div style={{ fontSize: 14, fontWeight: 500 }}>現場写真ビューア</div>
                  <div style={{ fontSize: 12, marginTop: 4 }}>写真を選択するか、新しい写真をアップロードしてください</div>
                </div>
              )}

              {activeTab === 'safety' && (
                <div style={{ textAlign: 'center', padding: 40, color: '#94a3b8' }}>
                  <Icon name="shield" size={40} style={{ color: '#d1d5db', margin: '0 auto 12px' }} />
                  <div style={{ fontSize: 14, fontWeight: 500 }}>安全管理パネル</div>
                  <div style={{ fontSize: 12, marginTop: 4 }}>KY活動記録・安全パトロール・ヒヤリハット</div>
                </div>
              )}

              {activeTab === 'daily' && (
                <div style={{ textAlign: 'center', padding: 40, color: '#94a3b8' }}>
                  <Icon name="file-text" size={40} style={{ color: '#d1d5db', margin: '0 auto 12px' }} />
                  <div style={{ fontSize: 14, fontWeight: 500 }}>作業日報</div>
                  <div style={{ fontSize: 12, marginTop: 4 }}>本日の作業日報を作成・確認できます</div>
                </div>
              )}
            </div>
          </div>
