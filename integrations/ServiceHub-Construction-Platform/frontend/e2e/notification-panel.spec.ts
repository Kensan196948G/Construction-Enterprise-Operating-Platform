...
test.describe("Notification Badge + Panel (Phase 4c)", () => {
  ...
  test("SSE 通知受信でバッジカウントが増加する", async ({ page }) => {
    // loginAndNavigate registers an empty SSE mock via setupAllApiMocks (LIFO last = highest priority).
    // We must first navigate, then unroute the empty mock and replace it with the notification mock
    // so that EventSource's next reconnect (after ~3s) delivers the notification.
    await loginAndNavigate(page);
    await page.unroute("**/api/v1/notifications/stream**");
    await mockSSEWithNotification(page);

    // EventSource fires onerror on the empty response, reconnects after ~3s, hits notification mock
  ...
  test("SSE 通知受信後にパネルを開くと通知が表示される", async ({ page }) => {
    ...
    // Wait for the notification to arrive (EventSource reconnects after ~3s)
  ...
  test("パネルを開くと未読カウントがクリアされる", async ({ page }) => {
    ...
    // Wait for unread badge (EventSource reconnects after ~3s)
    await expect(page.getByTestId("unread-count")).toBeVisible({ timeout: 10_000 });
