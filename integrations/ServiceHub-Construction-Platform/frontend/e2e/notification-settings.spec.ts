...
async function goToSettingsWithPrefs(
  ...
  // NOTE: sidebar label was updated from "設定" to "システム設定" in fcf5174
  await page.getByRole("link", { name: "システム設定", exact: true }).click();
  await page.waitForURL("**/settings");
  await expect(
    page.getByRole("heading", { name: "システム設定", level: 1 }),
  ).toBeVisible({ timeout: 10_000 });
  // Settings page opens on "profile" tab by default; navigate to the notifications tab
