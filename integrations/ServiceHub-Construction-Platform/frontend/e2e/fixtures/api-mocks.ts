...
export async function setupAllApiMocks(page: Page): Promise<void> {
  ...
  // Mock SSE notification stream (empty — no events delivered).
  // EventSource.onopen fires on status 200; onerror fires when the empty
  // body ends, but this avoids ECONNREFUSED which would freeze tests in CI
  // where no backend is running.
