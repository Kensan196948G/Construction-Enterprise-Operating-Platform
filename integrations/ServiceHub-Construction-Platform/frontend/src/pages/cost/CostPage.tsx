...
import { Badge, Button, Card, ErrorBanner, ErrorText, Skeleton, Modal, FormField, Input, Select, DataActions } from "@/components/ui";
...
export default function CostPage() {
  ...
  const exportData = {
    columns: ["記録日", "カテゴリ", "説明", "予算額(円)", "実績額(円)"],
    rows: records.map((r): CellValue[] => [
      r.record_date,
      CATEGORY_LABEL[r.category] ?? r.category,
      r.description,
      Number(r.budgeted_amount),
      Number(r.actual_amount),
    ]),
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <DataActions target="原価管理" exportData={exportData} />
          <Button
