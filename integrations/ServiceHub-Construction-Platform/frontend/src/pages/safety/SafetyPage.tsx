...
import { Badge, Button, Card, ErrorText, Skeleton, DataActions } from "@/components/ui";
...
export default function SafetyPage() {
  ...
  const exportData = tab === "checks"
    ? {
        columns: ["確認日", "確認区分", "合計項目", "OK", "NG", "総合結果", "備考"],
        rows: checks.map((c): CellValue[] => [
          c.check_date,
          c.check_type,
          c.items_total,
          c.items_ok,
    ...
    : {
        columns: ["検査日", "検査種別", "対象項目", "基準値", "計測値", "結果", "備考"],
        rows: inspections.map((i): CellValue[] => [
          i.inspection_date,
          i.inspection_type,
          i.target_item,
          i.standard_value ?? "",
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <DataActions target="安全品質" exportData={exportData} />
          <Button
