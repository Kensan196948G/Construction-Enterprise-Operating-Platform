...
import { Badge, Button, Card, Skeleton, Modal, FormField, Input, Select, Pagination, DataActions } from "@/components/ui";
...
export default function ProjectsPage() {
  ...
  const exportData = {
    columns: ["案件コード", "案件名", "施主", "ステータス", "開始日", "完了予定日"],
    rows: (filtered ?? []).map((p): CellValue[] => [
      p.project_code, p.name, p.client_name,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <DataActions target="工事案件" exportData={exportData} />
