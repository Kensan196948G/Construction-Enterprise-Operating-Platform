...
import { Badge, Button, Card, ErrorBanner, ErrorText, Skeleton, DataActions } from "@/components/ui";
...
export default function DailyReportsPage() {
  ...
  const exportData = {
    columns: ["報告日", "天気", "作業員数", "進捗率(%)", "安全確認", "ステータス"],
    rows: reports.map((r): CellValue[] => [
      r.report_date,
      WEATHER_LABEL[r.weather ?? ""] ?? r.weather ?? "",
      r.worker_count,
      r.progress_rate ?? 0,
      r.safety_check ? "確認済" : "未確認",
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <DataActions target="日報" exportData={exportData} />
          <Button
