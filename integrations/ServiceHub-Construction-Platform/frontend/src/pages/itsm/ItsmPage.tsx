...
import { Badge, Button, Card, DataActions, ErrorText, Modal, FormField, Input, Select, Textarea, Skeleton } from "@/components/ui";
...
export default function ItsmPage() {
  ...
  const exportData = tab === "incidents"
    ? {
        columns: ["番号", "タイトル", "カテゴリ", "優先度", "重大度", "ステータス"],
        rows: incidents.map((inc): CellValue[] => [
          inc.incident_number,
          inc.title,
          inc.category,
    ...
    : {
        columns: ["変更番号", "タイトル", "変更タイプ", "リスク", "ステータス"],
        rows: changes.map((cr): CellValue[] => [
          cr.change_number,
          cr.title,
          cr.change_type,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <DataActions target={tab === "incidents" ? "インシデント" : "変更要求"} exportData={exportData} />
