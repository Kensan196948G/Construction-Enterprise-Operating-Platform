...
import { Badge, Button, Card, DataActions, ErrorBanner, FormField, Input, Select, Skeleton } from "@/components/ui";
...
export default function PhotosPage() {
  ...
  const exportData = {
    columns: ["ファイル名", "カテゴリ", "説明", "サイズ(KB)", "登録日"],
    rows: filtered.map((photo): CellValue[] => [
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <DataActions target="写真・資料" exportData={exportData} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filtered.map((photo) => (
            ...
            >
              ...
              <div
                className="w-full h-40 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600 items-center justify-center flex-col gap-2 text-gray-400 dark:text-gray-500"
                style={{ display: photo.url ? "none" : "flex" }}
              >
                <span className="text-4xl">
                  {photo.category === "SAFETY" ? "🦺"
                   : photo.category === "PROGRESS" ? "🏗️"
                   : photo.category === "COMPLETION" ? "🏢"
                   : photo.category === "TROUBLE" ? "⚠️"
                   : "📷"}
                </span>
                <span className="text-xs font-medium">{CATEGORY_LABELS[photo.category]}</span>
              </div>
