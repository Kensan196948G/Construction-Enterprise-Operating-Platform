...
async function request<T>(
  ...
  // Mock mode: intercept before any real fetch
  if (MOCK_MODE) {
    let mockPath = path;
    if (options.params) {
    ...
    const mock = resolveMock(method, mockPath, body);
    if (mock) {
      if (!mock.status || mock.status >= 400) {
        throw new ApiError(
          mock.status ?? 500,
          { data: mock.data, status: mock.status ?? 500 },
          `Mock HTTP ${mock.status}`,
        );
      }
      return {
        data: mock.data as T,
    ...
    // Unmatched mock route — return empty success
    return { data: null as T, status: 200, headers: new Headers() };
  }
