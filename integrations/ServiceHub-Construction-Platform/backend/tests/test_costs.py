...
# ---------------------------------------------------------------------------
# 下請法60日支払期限チェック — IDOR 防止 (project-level access control)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_pm_can_run_legal_check_own_project(auth_client, pm_headers, pm_project):
    """PROJECT_MANAGER は自身が管理するプロジェクトの支払期限チェック実行可"""
    resp = await auth_client.get(
...
@pytest.mark.asyncio
async def test_pm_cannot_run_legal_check_other_project(auth_client, pm_headers):
    """PROJECT_MANAGER は管理外プロジェクトの支払期限チェック不可 (IDOR 防止)"""
    other_project_id = str(uuid.uuid4())
    resp = await auth_client.get(
        f"/api/v1/projects/{other_project_id}/cost-records/legal-check",
        headers=pm_headers,
    )
