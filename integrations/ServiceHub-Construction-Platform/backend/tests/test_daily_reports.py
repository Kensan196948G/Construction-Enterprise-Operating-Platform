...
@pytest.mark.asyncio
async def test_create_daily_report(auth_client, admin_headers, pm_project):
    """日報作成 - 正常"""
...
@pytest.mark.asyncio
async def test_list_daily_reports(auth_client, admin_headers, pm_project):
    """日報一覧取得"""
    project_id = str(pm_project.id)
    # 日報を2件作成
...
@pytest.mark.asyncio
async def test_get_daily_report(auth_client, admin_headers, pm_project):
    """日報詳細取得"""
    project_id = str(pm_project.id)
...
@pytest.mark.asyncio
async def test_update_daily_report(auth_client, admin_headers, pm_project):
    """日報更新"""
    project_id = str(pm_project.id)
    create_resp = await auth_client.post(
        f"/api/v1/projects/{project_id}/daily-reports",
...
@pytest.mark.asyncio
async def test_delete_daily_report(auth_client, admin_headers, pm_project):
    """日報削除（論理削除）"""
    project_id = str(pm_project.id)
    create_resp = await auth_client.post(
        f"/api/v1/projects/{project_id}/daily-reports",
...
@pytest.mark.asyncio
async def test_pm_can_create_daily_report(auth_client, pm_headers, pm_project):
    """PROJECT_MANAGERロールは自身が管理するプロジェクトの日報作成可 (IDOR 防止)"""
    project_id = str(pm_project.id)
    resp = await auth_client.post(
        f"/api/v1/projects/{project_id}/daily-reports",
        json={
...
@pytest.mark.asyncio
async def test_pm_cannot_create_daily_report_other_project(auth_client, pm_headers):
    """PROJECT_MANAGER は管理外プロジェクトの日報を作成できない (IDOR 防止)"""
    other_project_id = str(uuid.uuid4())
    resp = await auth_client.post(
        f"/api/v1/projects/{other_project_id}/daily-reports",
        json={
    ...
    # 存在しない/管理外プロジェクト → NotFound(404) または Forbidden(403)
