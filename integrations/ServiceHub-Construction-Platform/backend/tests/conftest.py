...
# Pre-compute hash using bcrypt directly to avoid passlib/bcrypt 5.0 incompatibility
# (passlib 1.7.4's detect_wrap_bug sends a 200+ byte password, bcrypt 5.0 rejects it)
_TEST_PW_HASH = _bcrypt.hashpw(b"TestPass1!", _bcrypt.gensalt(rounds=4)).decode()
...
# 固定テストプロジェクト UUID（PM_USER_ID が manager）
PM_PROJECT_ID = uuid.UUID("00000000-0000-0000-0000-0000000000a1")
# ADMIN が manager を務める別プロジェクト（PM からは 403 になる）
ADMIN_PROJECT_ID = uuid.UUID("00000000-0000-0000-0000-0000000000a2")


@pytest_asyncio.fixture
async def pm_project(db_session_with_users):
    """PM_USER_ID が manager を務めるプロジェクトをシードする。

    project-level IDOR 防止 (assert_project_access) のテストで、PM が自身の
    管理プロジェクトに対して書き込めることを検証するために使用する。
    auth_client と同一の db_session_with_users を共有するため、テストで両方を
    要求すれば同じセッション・同じプロジェクトを参照できる。
    """
    from app.models.project import Project

    project = Project(
        id=PM_PROJECT_ID,
        project_code="TEST-PJ-001",
        name="テスト工事案件",
        client_name="テスト発注者",
        manager_id=PM_USER_ID,
    )
    db_session_with_users.add(project)
    await db_session_with_users.commit()
    return project


@pytest_asyncio.fixture
async def admin_project(db_session_with_users):
    """ADMIN_USER_ID が manager を務める別プロジェクトをシードする。

    PM が他プロジェクトの情報にアクセスできないことを検証するために使用する。
    非存在 UUID（404）ではなく実在する別プロジェクト（403）を使ったテストで
    IDOR 防止の正確性を担保する。
    """
    from app.models.project import Project
    ...
    project = Project(
        id=ADMIN_PROJECT_ID,
        project_code="TEST-PJ-ADMIN-002",
        name="ADMIN 管理プロジェクト",
        client_name="テスト発注者B",
