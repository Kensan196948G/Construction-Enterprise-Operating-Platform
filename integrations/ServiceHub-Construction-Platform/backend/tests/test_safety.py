...
@pytest.mark.asyncio
async def test_create_safety_check(auth_client, admin_headers, pm_project):
    """安全点検作成 - 正常"""
...
@pytest.mark.asyncio
async def test_list_safety_checks(auth_client, admin_headers, pm_project):
    """安全点検一覧取得"""
    project_id = str(pm_project.id)
    # 2件作成
...
@pytest.mark.asyncio
async def test_pm_can_create_safety_check(auth_client, pm_headers, pm_project):
    """PROJECT_MANAGERは自身が管理するプロジェクトの安全点検作成可 (IDOR 防止)"""
    project_id = str(pm_project.id)
    resp = await auth_client.post(
        f"/api/v1/projects/{project_id}/safety-checks",
        json={
...
@pytest.mark.asyncio
async def test_pm_cannot_create_safety_check_other_project(auth_client, pm_headers):
    """PROJECT_MANAGER は管理外プロジェクトの安全点検を作成できない (IDOR 防止)"""
    other_project_id = str(uuid.uuid4())
    resp = await auth_client.post(
        f"/api/v1/projects/{other_project_id}/safety-checks",
        json={
...
@pytest.mark.asyncio
async def test_create_quality_inspection(auth_client, admin_headers, pm_project):
    """品質検査作成 - 正常"""
    project_id = str(pm_project.id)
    resp = await auth_client.post(
        f"/api/v1/projects/{project_id}/quality-inspections",
...
@pytest.mark.asyncio
async def test_list_quality_inspections(auth_client, admin_headers, pm_project):
    """品質検査一覧取得"""
    project_id = str(pm_project.id)
