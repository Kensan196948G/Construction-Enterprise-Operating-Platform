...
from app.schemas.legal import ContractDeadlineAlert, LegalDeadlineNotification
...
@router.get(
    "/legal-deadlines",
    response_model=ApiResponse[LegalDeadlineNotification],
)
async def get_legal_deadline_notifications(
    db: Annotated[AsyncSession, Depends(get_db)],
    current_user: Annotated[
        User,
        Depends(require_roles(UserRole.ADMIN, UserRole.PROJECT_MANAGER)),
    ],
    days: int = Query(default=90, ge=1, le=365, description="期限切れまでの日数上限"),
) -> ApiResponse[LegalDeadlineNotification]:
    """契約期限アラート一覧を返す。

    days パラメータ以内に期限が切れる ACTIVE 契約を urgency 別に集計して返す。
    urgency 区分: CRITICAL(<=7d) / HIGH(<=30d) / MEDIUM(<=60d) / LOW(<=90d)
    """
    ...
    rows = await db.execute(
        ...
        .where(
            ...
            Contract.period_end <= cutoff,
            Project.deleted_at.is_(None),
        )
        .order_by(Contract.period_end)
    )

    alerts: list[ContractDeadlineAlert] = []
    for row in rows.all():
        remaining = (row.period_end - now.date()).days
    ...
    return ApiResponse(
        data=LegalDeadlineNotification(
            total_alerts=len(alerts),
            critical_alerts=critical,
            high_alerts=high,
            medium_alerts=medium,
            low_alerts=low,
            contract_deadlines=alerts,
            generated_at=now,
        )
    )
