...
class ProjectRepository:
    ...
    async def list_manager_project_ids(self, user_id: uuid.UUID) -> list[uuid.UUID]:
        """指定ユーザーが manager を務める（削除されていない）プロジェクト ID 一覧。

        IDOR 防止: 非 ADMIN ユーザーの一覧系エンドポイントを自身の管理
        プロジェクトに限定するスコープ集合として使用する。
        """
        result = await self.db.execute(
            select(Project.id).where(
                Project.manager_id == user_id, Project.deleted_at.is_(None)
            )
        )
