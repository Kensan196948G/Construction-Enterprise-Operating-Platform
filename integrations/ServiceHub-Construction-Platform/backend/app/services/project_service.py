...
class ProjectService:
    ...
    async def create_project(
        ...
        # manager_id 未指定時は作成者を既定の manager とする。
        # manager_id は project-level アクセス制御 (assert_project_access) の
        # 所有者シグナルであり、未設定だと作成者ですら自プロジェクトを操作
        # できない孤児プロジェクトになるため。
        if data.manager_id is None:
            data = data.model_copy(update={"manager_id": created_by})
