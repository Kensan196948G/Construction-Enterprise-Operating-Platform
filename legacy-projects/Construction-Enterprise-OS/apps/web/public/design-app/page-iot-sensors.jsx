...
function IoTSensorsView() {
  ...
  const chartData = React.useMemo(() => {
    ...
    let val = base;
    for (let i = 0; i < 24; i++) {
      val += (Math.random() - 0.48) * base * 0.15;
        ...
        {sensors.map((s, i) => (
          <div
            ...
            style={{
              ...
              padding: 14,
              cursor: "pointer",
              borderColor: selectedSensor === i ? "#1a56db" : "#e2e8f0",
          ...
          >
            ...
            >
              ...
              >
                <Icon
                  name={sensorIcon[s.type] || "activity"}
                  size={16}
        ...
        <div style={cs}>
          ...
          >
            <div>
              <span style={{ fontSize: 14, fontWeight: 700, color: "#0f172a" }}>
                {sensors[selectedSensor].name}
              </span>
              <span style={{ fontSize: 12, color: "#94a3b8", marginLeft: 8 }}>
                {sensors[selectedSensor].location}
              </span>
            </div>
              ...
              {["1h", "6h", "24h", "7d"].map((t) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 4,
                    border: "none",
                    background: timeRange === t ? "#1a56db" : "#f1f5f9",
          ...
          <div style={{ padding: "20px 20px 10px" }}>
            ...
            >
              ...
              <path
                d={
                  ...
                  chartData
                    .map(
                      (v, i) =>
                        `L${(i / (chartData.length - 1)) * 600},${200 - ((v - minVal) / range) * 170}`,
                    )
                    .join(" ") +
                  ` L600,200 L0,200 Z`
                }
              ...
              <circle
                ...
                cy={
                  200 -
                  ((chartData[chartData.length - 1] - minVal) / range) * 170
                }
                r="4"
                fill="#1a56db"
                stroke="#fff"
          ...
          >
            {[
              [
                "現在値",
                sensors[selectedSensor].value + sensors[selectedSensor].unit,
              ],
              ["最大", maxVal.toFixed(1) + sensors[selectedSensor].unit],
              ["最小", minVal.toFixed(1) + sensors[selectedSensor].unit],
              [
                "平均",
                (
                  chartData.reduce((a, b) => a + b, 0) / chartData.length
                ).toFixed(1) + sensors[selectedSensor].unit,
              ],
            ].map((s, i) => (
              <div key={i} style={{ textAlign: "center" }}>
                <div style={{ fontSize: 10, color: "#94a3b8" }}>{s[0]}</div>
                <div
                  style={{
                    fontSize: 14,
                    fontWeight: 600,
