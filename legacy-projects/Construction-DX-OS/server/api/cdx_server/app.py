...
def create_app(
    ...
    _DESCRIPTION = """
## Construction DX OS — Central Platform API

建設現場 DX 統合プラットフォームの REST API です。
デバイス登録・ハートビート・インベントリ収集・PXE ブート・ISO ビルド・シリアルスキャンを一元管理します。

### 認証

- **デバイス登録**: `X-Registration-Token` ヘッダー (環境変数 `CDX_REGISTRATION_TOKEN`)
- **Admin UI**: HTTP Basic Auth (`CDX_ADMIN_PASSWORD`) または OIDC (`AUTH_BACKEND=oidc`)
- **メトリクス**: 認証不要 (内部ネットワーク専用)

"""

    _TAGS_METADATA = [
        {"name": "devices", "description": "デバイス登録・一覧・詳細取得・削除"},
        {"name": "heartbeat", "description": "デバイスからのハートビート受信 (死活監視)"},
        {"name": "inventory", "description": "デバイスインベントリ (ハードウェア情報) 収集"},
        {"name": "health", "description": "ヘルスチェック — `/health/live` (liveness) / `/health/ready` (readiness)"},
        {"name": "dashboard", "description": "ダッシュボード統計 API"},
        {"name": "pxe", "description": "PXE ブートイベント受信・ロールバック管理"},
        {"name": "iso-builds", "description": "ISO イメージビルドジョブ管理"},
        {"name": "serial-scan", "description": "シリアル番号スキャンパイプライン (GMSV0002)"},
        {"name": "registration", "description": "デバイス登録トークン管理"},
        {"name": "policy", "description": "デバイスポリシー設定"},
        {"name": "admin", "description": "管理 UI — デバイス管理・ISO ビルダー (Basic Auth 必須)"},
    ]

    app = FastAPI(
        title="Construction DX OS API",
        version=__version__,
        description=_DESCRIPTION,
        openapi_tags=_TAGS_METADATA,
        contact={
            "name": "Construction DX OS Development Team",
            "url": "https://github.com/Kensan196948G/Construction-DX-OS",
        },
        license_info={
            "name": "MIT",
